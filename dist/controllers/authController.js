"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __spreadArray = (this && this.__spreadArray) || function (to, from, pack) {
    if (pack || arguments.length === 2) for (var i = 0, l = from.length, ar; i < l; i++) {
        if (ar || !(i in from)) {
            if (!ar) ar = Array.prototype.slice.call(from, 0, i);
            ar[i] = from[i];
        }
    }
    return to.concat(ar || Array.prototype.slice.call(from));
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
// Models
var user_1 = __importDefault(require("../models/user"));
// Utils
var authUtils_1 = require("../libs/authUtils");
var chainsUtils_1 = require("../libs/chainsUtils");
var utils_1 = require("../libs/utils");
var constants_1 = require("../libs/constants");
exports.sign_up_post = __spreadArray(__spreadArray([], chainsUtils_1.signUpValidationChain, true), [
    function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var validationResult, _a, first_name, last_name, username, password, hashPassword, newUser, newUserResult, err_1;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        _b.trys.push([0, 3, , 4]);
                        validationResult = (0, utils_1.checkValidationErrors)(req);
                        if (validationResult) {
                            return [2 /*return*/, res.status(404).json({
                                    context: constants_1.SIGN_UP,
                                    errors: validationResult
                                })];
                        }
                        _a = req.body, first_name = _a.first_name, last_name = _a.last_name, username = _a.username, password = _a.password;
                        return [4 /*yield*/, (0, authUtils_1.genPassword)(password)];
                    case 1:
                        hashPassword = _b.sent();
                        if (hashPassword.includes('Error:')) {
                            return [2 /*return*/, res.status(404).json({
                                    context: constants_1.SIGN_UP,
                                    message: 'GEN PASSWORD: Error when trying to hash password',
                                    errors: [hashPassword]
                                })];
                        }
                        newUser = new user_1.default({
                            first_name: first_name,
                            last_name: last_name,
                            username: username,
                            password: hashPassword,
                            timestamp: new Date(),
                            friends: [],
                            friend_requests: []
                        });
                        return [4 /*yield*/, newUser.save()];
                    case 2:
                        newUserResult = _b.sent();
                        return [2 /*return*/, res.status(200).json({
                                context: constants_1.SIGN_UP,
                                message: 'User signed up successfully',
                                user: newUserResult
                            })];
                    case 3:
                        err_1 = _b.sent();
                        console.log('SIGN UP: Error while trying to save new user in db');
                        console.log(err_1);
                        if (err_1._message === 'User validation failed') {
                            return [2 /*return*/, res.status(404).json({
                                    context: constants_1.SIGN_UP,
                                    message: 'SIGN UP: User already exists',
                                    errors: ['User already exists']
                                })];
                        }
                        return [2 /*return*/, res.status(500).json({
                                context: constants_1.SIGN_UP,
                                message: 'SIGN UP: Error while trying to save new user in db',
                                errors: [err_1.message]
                            })];
                    case 4: return [2 /*return*/];
                }
            });
        });
    }
], false);
exports.log_in_post = __spreadArray(__spreadArray([], chainsUtils_1.logInValidationChain, true), [
    function (req, res) {
        return __awaiter(this, void 0, void 0, function () {
            var _a, username, password, validationResult, foundUser, _b, token, expiresIn, err_2;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        _c.trys.push([0, 3, , 4]);
                        _a = req.body, username = _a.username, password = _a.password;
                        validationResult = (0, utils_1.checkValidationErrors)(req);
                        if (validationResult) {
                            return [2 /*return*/, res.status(404).json({
                                    context: constants_1.LOG_IN,
                                    errors: validationResult
                                })];
                        }
                        return [4 /*yield*/, user_1.default.findOne({ username: username })];
                    case 1:
                        foundUser = _c.sent();
                        if (!foundUser) {
                            return [2 /*return*/, res.status(404).json({
                                    context: constants_1.LOG_IN,
                                    message: 'LOG IN: User not found',
                                    errors: ['User not found']
                                })];
                        }
                        return [4 /*yield*/, (0, authUtils_1.checkValidPassword)(foundUser.password, password)];
                    case 2:
                        if (_c.sent()) {
                            _b = (0, authUtils_1.issueJWT)(foundUser._id), token = _b.token, expiresIn = _b.expiresIn;
                            return [2 /*return*/, res.status(200).json({
                                    context: constants_1.LOG_IN,
                                    message: 'Log in successful',
                                    token: token,
                                    expiresIn: expiresIn,
                                    user: foundUser
                                })];
                        }
                        else {
                            return [2 /*return*/, res.status(404).json({
                                    context: constants_1.LOG_IN,
                                    message: 'LOG IN: Entered wrong password',
                                    errors: ['Entered wrong password']
                                })];
                        }
                        return [3 /*break*/, 4];
                    case 3:
                        err_2 = _c.sent();
                        console.log('LOG IN: Error while trying to log in user');
                        console.log(err_2);
                        res.status(500).json({
                            context: constants_1.LOG_IN,
                            message: 'LOG IN: Error while trying to log in user',
                            errors: [err_2.message]
                        });
                        return [3 /*break*/, 4];
                    case 4: return [2 /*return*/];
                }
            });
        });
    }
], false);
